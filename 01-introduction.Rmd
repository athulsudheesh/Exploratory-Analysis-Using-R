...
## What is Multivariate Statisitcs ?
...
There are many different models, each with it's own type of analysis. In this textbook we will be dealing mostly with the following ones [@MultiVariateAnalysis]:
...
require(GGally)
#Loading datasets
WorldHealthVarDesc <- read.csv("WorldHealthVarDesc.csv")
WorldHealth <- read.csv("Data/WorldHealth.csv", row.names = 1)
Orange_rating<- read.csv("Data/44JudgesDescribe10OrangeJuices-CA.csv", row.names = 1)
...
```{r WorldHealthVarDesc, echo=FALSE, tidy.opts=TRUE}
# kable is a function for printing neat tables 
kable(WorldHealthVarDesc, 
      booktabs = TRUE,
      caption = 'Variables from World Health dataset ', 
      format = "latex"
) %>% 
